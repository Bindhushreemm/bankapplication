...
public class phoneUpdate extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(mpin.equals(mpin1) ) {
			CustomerDao.UpdatePhoneUser(user1, mpin,pno);
			
//			request.getSession().setAttribute("credentials", "credentialsnotexist");
			RequestDispatcher rd=request.getRequestDispatcher("updatephono.jsp?flag="+true);
			...
			RequestDispatcher rd=request.getRequestDispatcher("updatephono.jsp");
			rd.include(request, response);
